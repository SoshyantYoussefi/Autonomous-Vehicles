        ...
        for xi, di in zip(neighbours, distances):
            ...
            if new_cost < costs[xi]:
                ...
                if queue.ismember(xi):
                    ...
                    # Öppna även tidigare noder
                    queue.insert(xi, priority)
...
def measure(mission, method, factors):
    ...
    if method == "reuse":
        ...
        for c in factors:
            ...
            # Kumulativ tid
            stages.append(perf_counter() - start)
...
if __name__ == "__main__":
    ...
    for i, mission in enumerate(source["pre_mission"]):
        ...
        # Värm upp
        for method in methods:
            measure(mission, method, factors)

        for repeat in range(repeats):
            order = methods[repeat % 3:] + methods[:repeat % 3]

            # Variera körordningen
            for method in order:
                elapsed, stages, result = measure(
                    mission,
                    method,
                    factors
                )
                ...
                length = sum(
                    float(source["osm_map"].distancematrix[a, b])
                ...
                stage_samples[method].append(
                    np.asarray(stages) * 1000
                )

        medians = {
            method: float(np.median(samples[method]))
            for method in methods
        }
        ...
        row["saved_percent"] = (
            100 * (1 - medians["reuse"] / medians["restart"])
        )

        row["speedup"] = (
            medians["restart"] / medians["reuse"]
        )
        ...
        print(
            f"Mission {i}: "
            f"ARA* {medians['reuse']:.1f} ms, "
            f"omstarter {medians['restart']:.1f} ms, "
            f"A* {medians['astar']:.1f} ms, "
            f"sparat {row['saved_percent']:.1f} %, "
            f"{row['speedup']:.2f}x",
            flush=True
        )
