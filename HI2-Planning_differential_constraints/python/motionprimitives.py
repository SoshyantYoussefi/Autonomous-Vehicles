...
class MotionPrimitives:
    ...
    def plan_to_path(self, x0, plan):
        ...
        arguments:
            x0 -- Coordinates of initial state [x, y, theta]
        ...
        for ui in control:
            ...
            if ui[2] == 1:  # Forward driving
                ...
                traj = np.column_stack(
                    (
                        p0[0:2] + np.column_stack((np.flip(mpi["x"]) - mpi["x"][-1], np.flip(mpi["y"]) - mpi["y"][-1])),
                        np.flip(mpi["th"]),
                    )
                )
    ...
    def generate_primitives(self, theta_init, state_0, L=1.5, v=15, u_max=np.pi / 4, print_level=3):
        ...
        """
        self.mprims = compute_motion_primitives(theta_init, state_0, L, v, u_max, print_level)
...
def compute_motion_primitives(theta_init, state_0, L, v, u_max, print_level=3):
    ...
    def compute_mprim(state_i, lattice, L, v, u_max, print_level):
        ...
        for state_f in lattice:
            ...
            F = ca.Function("f", [x, u], [v * ca.cos(x[2]), v * ca.sin(x[2]), v * ca.tan(u) / L])
            ...
            opti.solver(
                "ipopt", {"expand": True, "print_time": print_level > 0}, {"tol": 10**-8, "print_level": print_level}
            )
            sol = opti.solve()
            ...
            u_opt = sol.value(U)
            T_opt = sol.value(T)
            mprim.append({"x": pos_x_opt, "y": pos_y_opt, "th": ang_th_opt, "u": u_opt, "T": T_opt, "ds": T_opt * v})
    ...
    xy_pi_4 = np.sqrt(2) * (state_0[:, 0:2] @ np.array([[np.cos(dth), np.sin(dth)], [-np.sin(dth), np.cos(dth)]]))
    ...
    for i in range(2, len(theta_init)):
        ...
        for j in range(len(mprims[i])):
            mp = mprims[i][j]
            xy_vec_rot = np.array([[np.cos(th), -np.sin(th)], [np.sin(th), np.cos(th)]]) @ np.vstack((mp["x"], mp["y"]))
