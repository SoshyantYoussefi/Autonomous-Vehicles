...
class BoxWorld:
    ...
    def add_box(self, x, y, width, height, fill_box=True):
        ...
        self.x_obst = np.vstack((self.x_obst, [x, x + width]))
        self.y_obst = np.vstack((self.y_obst, [y, y + height]))
    ...
    def draw_box(self, b, *args, ax=None, **kwargs):
        ...
        if fill_box:
            ax.fill([x0, x0 + W1, x0 + W1, x0, x0], [y0, y0, y0 + W2, y0 + W2, y0], *args, **kwargs)
        else:
            ax.plot([x0, x0 + W1, x0 + W1, x0, x0], [y0, y0, y0 + W2, y0 + W2, y0], *args, **kwargs)
...
def state_space_from_lattice(lattice):
    ...
    if len(lattice) == 1:
        ...
        for xi in lattice[0]:
            st_sp = np.hstack((st_sp, np.vstack((np.full((1, N), xi), st_sp_1))))
    return st_sp
...
def obstacle_check(x, y, x_obst, y_obst):
    ...
    for ii in range(x_obst.shape[0]):
        if (x > x_obst[ii, 0] and x < x_obst[ii, 1]) and (y > y_obst[ii, 0] and y < y_obst[ii, 1]):
            return True
    return False
