...
class SplinePath(PathBase):
    def __init__(self, p, min_grid=None):
        ...
        """
        si = np.hstack(([0], np.cumsum(np.sqrt(np.sum(np.diff(p[:, 0:2], axis=0) ** 2, axis=1)))))
    ...
    def project(
        self, p: np.ndarray, s0: float, ds: float = 1, s_lim: int = 20, verbose: bool = False
    ) -> tuple[float, float]:
        ...
        dp = np.cross(np.hstack((hi, 0.0)), np.hstack((dp, 0.0)))[2]
        return si, dp
