...
class VehicleModelBase(ABC):
    ...
    def set_attributes(self, opts: dict):
    ...
    def simulate(self, w0: np.ndarray, T: float, dt: float,
...
class SingleTrackModel(VehicleModelBase):
    ...
    def dx(self, t: float, w: np.ndarray, u: np.ndarray) -> np.ndarray:
        ...
        w : np.ndarray
            state (x, y, theta, v)
        u : np.ndarray
            control input (delta, acceleration)
